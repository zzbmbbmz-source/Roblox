...
function MyUI:CreateWindow(config)
    ...
    local Window = setmetatable({
        ...
        _isOpen = true,
        _tabOrder = 0, -- ตัวนับลำดับ tab อัตโนมัติ ใช้กำหนดตำแหน่งซ้าย-ขวา/บน-ล่างของแท็บ
    }, { __index = MyUI })
...
-- ===== Tab =====
-- order (ไม่บังคับ): ตัวเลขกำหนดตำแหน่งแท็บ ค่ายิ่งน้อยยิ่งอยู่บน
-- ถ้าไม่ระบุ จะต่อท้ายรายการอัตโนมัติ (เหมือนเดิม)
-- ตัวอย่าง: Window:CreateTab("My Feature", 0) จะถูกดันขึ้นไปอยู่บนสุด
-- เพราะแท็บที่สร้างอัตโนมัติเริ่มที่ 10, 20, 30, ... (มากกว่า 0 เสมอ)
function MyUI:CreateTab(name, order)
    local isFirst = self._firstTab

    self._tabOrder = self._tabOrder + 10
    local layoutOrder = order or self._tabOrder
